    ...
    initDatabase() {
        try {
            ...
            this.db.serialize(() => {
                ...
                // Migration: Add nodes_with_critical_issues column if it doesn't exist
                this.db.run(`
    ...
    async storeCodeQualityMetrics(flowId, flowName, totalIssues, nodesWithIssues, nodesWithCriticalIssues, totalFunctionNodes, issueTypes, qualityScore, complexityScore) {
        return this.executeOperation(() => {
            return new Promise((resolve, reject) => {
                ...
                // Delete any existing entries for this flow to prevent duplicates
                this.db.run("DELETE FROM code_quality_metrics WHERE flow_id = ?", [flowId], (deleteErr) => {
                    if (deleteErr) {
                        reject(deleteErr);
                        return;
                    }
    ...
    async storeNodeQualityMetricsBatch(flowId, nodeMetrics) {
        return this.executeOperation(() => {
            return new Promise((resolve, reject) => {
                ...
                // Use a transaction for batch insert with duplicate prevention
                const db = this.db; // Capture database reference to avoid scope issues
                
                db.serialize(() => {
                    db.run("BEGIN TRANSACTION");
                    
                    // First delete any existing node records for this flow
                    db.run("DELETE FROM node_quality_metrics WHERE flow_id = ?", [flowId]);
