...
class PerformanceDatabase {
    ...
    initDatabase() {
        try {
            ...
            this.db.serialize(() => {
                ...
                // Create code quality metrics tables
                this.db.run(`
                ...
                this.db.run(`
                    CREATE TABLE IF NOT EXISTS quality_trends (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        metric_name TEXT NOT NULL, -- 'overall_quality', 'technical_debt', 'complexity'
                ...
                this.db.run('CREATE INDEX IF NOT EXISTS idx_code_quality_created_at ON code_quality_metrics(created_at)');
                this.db.run('CREATE INDEX IF NOT EXISTS idx_code_quality_flow ON code_quality_metrics(flow_id)');
                this.db.run('CREATE INDEX IF NOT EXISTS idx_node_quality_created_at ON node_quality_metrics(created_at)');
                this.db.run('CREATE INDEX IF NOT EXISTS idx_node_quality_flow_node ON node_quality_metrics(flow_id, node_id)');
                this.db.run('CREATE INDEX IF NOT EXISTS idx_quality_trends_created_at ON quality_trends(created_at)');
                this.db.run('CREATE INDEX IF NOT EXISTS idx_quality_trends_metric ON quality_trends(metric_name)');
    ...
    async pruneOldData(retentionDays = null) {
        return new Promise((resolve, reject) => {
            ...
            this.db.serialize(() => {
                ...
                // Delete old quality trends
                this.db.run('DELETE FROM quality_trends WHERE created_at < ?', [cutoffTime], (err) => {
                    if (err) {
                        this.db.run('ROLLBACK');
                        reject(new Error('Error pruning old quality trends: ' + err.message));
                        return;
                    }
                });
    ...
    async storeCodeQualityMetrics(flowId, flowName, totalIssues, nodesWithIssues, totalFunctionNodes, issueTypes, qualityScore, complexityScore) {
        return new Promise((resolve, reject) => {
            if (!this.db || !this.initialized) {
                reject(new Error('Database not initialized'));
                return;
            ...
            const stmt = this.db.prepare(`
                INSERT INTO code_quality_metrics (
                    flow_id, flow_name, total_issues, nodes_with_issues, total_function_nodes, 
                    issue_types, quality_score, complexity_score
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `);
            ...
            stmt.run(flowId, flowName, totalIssues, nodesWithIssues, totalFunctionNodes, 
                    issueTypesJson, roundedQualityScore, roundedComplexityScore, function(err) {
                stmt.finalize();
                if (err) {
                    reject(err);
                } else {
    ...
    async storeQualityTrend(metricName, metricValue, flowCount = 0, affectedNodes = 0) {
        return new Promise((resolve, reject) => {
            if (!this.db || !this.initialized) {
                reject(new Error('Database not initialized'));
                return;
            ...
            const stmt = this.db.prepare(`
                INSERT INTO quality_trends (metric_name, metric_value, flow_count, affected_nodes)
                VALUES (?, ?, ?, ?)
            `);
            
            const roundedMetricValue = Math.round(metricValue * 100) / 100;
            
            stmt.run(metricName, roundedMetricValue, flowCount, affectedNodes, function(err) {
                stmt.finalize();
                if (err) {
                    reject(err);
                } else {
    ...
    async getQualitySummary() {
        return new Promise((resolve, reject) => {
            ...
            this.db.all(`
                ...
                INNER JOIN (
                    SELECT flow_id, MAX(created_at) as max_created_at
                    FROM code_quality_metrics
                    GROUP BY flow_id
                ) latest ON cqm.flow_id = latest.flow_id AND cqm.created_at = latest.max_created_at
    ...
    async getQualityTrendAnalysis(metricName = 'overall_quality', hours = 24) {
        return new Promise((resolve, reject) => {
            if (!this.db || !this.initialized) {
                reject(new Error('Database not initialized'));
                return;
            ...
            `, [metricName, since], (err, rows) => {
                if (err) {
                    reject(err);
                } else if (rows.length < 2) {
                    resolve({ trend: 'insufficient_data', data: rows });
                } else {
                    const firstHalf = rows.slice(0, Math.floor(rows.length / 2));
                    const secondHalf = rows.slice(Math.floor(rows.length / 2));
                    
                    const firstAvg = firstHalf.reduce((sum, row) => sum + row.metric_value, 0) / firstHalf.length;
                    const secondAvg = secondHalf.reduce((sum, row) => sum + row.metric_value, 0) / secondHalf.length;
                    
                    const difference = secondAvg - firstAvg;
                    const threshold = 5; // 5 point threshold
                    
                    let trend;
                    if (metricName === 'overall_quality') {
                        trend = difference > threshold ? 'improving' : 
                                difference < -threshold ? 'degrading' : 'stable';
                    } else {
                        trend = difference > threshold ? 'increasing' : 
                                difference < -threshold ? 'decreasing' : 'stable';
                    }
                    
                    resolve({
                        trend: trend,
                        change: Math.round(difference * 100) / 100,
                        currentValue: Math.round(rows[rows.length - 1].metric_value * 100) / 100,
                        data: rows.map(row => ({
                            ...row,
                            metric_value: Math.round(row.metric_value * 100) / 100
                        }))
                    });
                }
            });
