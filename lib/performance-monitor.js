...
class PerformanceMonitor {
    constructor(database = null) {
        ...
        this.db = database || new PerformanceDatabase();
        if (this.db) {
    ...
    async pruneOldData() {
        if (!this.db) return { message: 'Database disabled' };
        try {
    ...
    async calculateAverages(windowMinutes = 10) {
        if (!this.db) return { cpu: 0, memory: 0, eventLoop: 0 };
        try {
    ...
    async checkSustainedMetric(metricColumn, threshold, duration) {
        if (!this.db) return { sustained: false, ratio: 0, totalReadings: 0, exceedingReadings: 0 };
        try {
    ...
    async getTrend(metricType, windowMinutes = 60) {
        if (!this.db) return 'insufficient_data';
        try {
    ...
    async getDatabaseStats() {
        if (!this.db) return { error: 'Database disabled' };
        try {
