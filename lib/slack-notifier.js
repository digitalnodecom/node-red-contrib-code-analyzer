class SlackNotifier {
    ...
    async sendPerformanceAlert(performanceSummary, fallbackCallback = null) {
        ...
        alerts.forEach(alert => {
            ...
            thresholdViolations.push(`**${metricName}** threshold of ${alert.threshold}${unit} (current: ${alert.current.toFixed(1)}${unit})`);
        });
        ...
        // Add current performance metrics for context
        message += '**Current System State**\n';
        message += `   • CPU Usage: ${current.cpu.toFixed(1)}%\n`;
        message += `   • Memory Usage: ${current.memory.toFixed(1)}%\n`;
        message += `   • Event Loop Lag: ${current.eventLoopLag ? current.eventLoopLag.toFixed(1) : 'N/A'}ms\n\n`;
        
        // Add process information
        message += '**System Information**\n';
        message += `   • Process ID: ${processInfo.pid}\n`;
        message += `   • Node.js Version: ${processInfo.nodeVersion}\n`;
        message += `   • Platform: ${processInfo.platform} (${processInfo.arch})\n`;
        message += `   • System Uptime: ${uptime.formatted}\n\n`;
