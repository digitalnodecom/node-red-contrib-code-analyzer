...
function parseIgnoreDirectives(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (trimmed.match(/\/\/\s*@nr-anal[yi]zer?-ignore-start/i)) {
            // Find the corresponding end directive
            for (let j = i + 1; j < lines.length; j++) {
                const endLine = lines[j].trim();
                if (endLine.match(/\/\/\s*@nr-anal[yi]zer?-ignore-end/i)) {
        ...
        if (trimmed.match(/\/\/\s*@nr-anal[yi]zer?-ignore-line/i)) {
            ignoreLines.add(i + 1); // 1-based line numbers
        }
        
        // Check for ignore-next directive
        if (trimmed.match(/\/\/\s*@nr-anal[yi]zer?-ignore-next/i)) {
            if (i + 1 < lines.length) {
