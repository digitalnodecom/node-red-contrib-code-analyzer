...
function detectDebuggingTraits(code, level = 1, options = {}) {
    ...
    try {
        ...
        // Fall back to regex-based detection if AST fails
        if (options.verbose) {
            console.warn('AST detection failed, falling back to regex:', error.message);
        }
        
        const result = detectDebuggingTraitsRegex(code, level);
